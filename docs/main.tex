...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\footnotesize Gwen OS --- Especificación de Requerimientos}
...
\title{\textbf{Gwen OS}\\[0.3cm]\Large Copiloto personal de Segundo Cerebro y Estudio\\[0.2cm]\large Especificación de Requerimientos de Software (SRS)}
...
\centering
\vspace*{3cm}
{\Huge\bfseries Gwen OS \par}
\vspace{0.5cm}
{\Large Copiloto personal de gestión de conocimiento y estudio \par}
\vspace{1.5cm}
...
RNF3 & Las respuestas conversacionales deben evitar \emph{cold-starts} del LLM (\texttt{keep\_alive}) manteniendo latencia aceptable. & Rendimiento & Media \\
...
Gemini y NotebookLM excluidos del flujo automático & RNF2 exige LLM local por defecto; NotebookLM además carece de API pública de consumo, por lo que su uso permanece manual. \\
...
Módulos de Finanzas y usuario secundario fuera de alcance & No formaban parte de la reconstrucción del proyecto desde cero; quedan como backlog condicionado a que RNF1 (escalabilidad) se cumpla en la práctica. \\
\bottomrule
\end{longtable}
...
\begin{enumerate}[leftmargin=1.2cm]
  \item Implementación del perfil de Hermes para \texttt{AgenteCurador} en modo revisión (RF5.4).
  \item Configuración de los workflows de n8n heredados (sincronización PKM y clasificador de inbox) apuntando al nuevo orquestador.
  \item Migración de la página \texttt{Proyecto} en Notion para reflejar esta especificación como fuente de verdad vigente.
  \item Plan de pruebas de aceptación por cada RF, derivado de las prioridades DINO ya asignadas.
\end{enumerate}
